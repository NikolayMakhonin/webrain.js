...
import {clearCallStates} from "../rx/depend/src/_helpers";
...
describe('common > main > lists > DependMap', function() {
	...
	it('base', function() {
		...
		depend(() => { assert.strictEqual(map.size, 0) })()
		...
		depend(() => { assert.strictEqual(map.get(2), void 0) })()
		...
		depend(() => { assert.deepStrictEqual(Array.from(map.entries()), [[1, 2]]) })()
