...
export interface ITupleIdMap extends Map<any, any> {
	getTupleIdMap(item, ...items): ITupleIdMap
	getTupleIdMap(this: ITupleIdMap, item, ...items): ITupleIdMap

	getTupleId(item, ...items): number
	getTupleId(this: ITupleIdMap, item, ...items): number
}
...
const mapRoot: ITupleIdMap = new Map() as ITupleIdMap
mapRoot.getTupleIdMap = getTupleIdMap
mapRoot.getTupleId = getTupleId

export function getTupleIdMap(item, ...items): ITupleIdMap
export function getTupleIdMap(this: ITupleIdMap, item, ...items): ITupleIdMap
export function getTupleIdMap(this: ITupleIdMap): ITupleIdMap {
	...
	for (let i = 0, len = arguments.length; i < len; i++) {
		...
		if (!nextMap) {
			nextMap = new Map() as ITupleIdMap
...
export function getTupleId(item, ...items): number
export function getTupleId(this: ITupleIdMap, item, ...items): number
export function getTupleId(this: ITupleIdMap): number {
	let map = this || mapRoot
