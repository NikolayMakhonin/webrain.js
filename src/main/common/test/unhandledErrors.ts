...
function browserUnhandledErrors(callback: (...args) => void) {
	const errorHandler = (...args) => {
		this.error(
			'unhandledrejection',
			...args.map(arg => arg instanceof PromiseRejectionEvent && arg.reason || arg),
		)
	}
...
function nodeUnhandledErrors(callback: (...args) => void) {
	process
...
function interceptEval(callback: (...args) => void) {
...
export function unhandledErrors(callback: (...args) => void) {
	if (isNode()) {
		// is node
