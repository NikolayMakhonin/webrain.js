...
import { ALWAYS_CHANGE_VALUE, getCallState, getOrCreateCallState, invalidateCallState } from '../../../../../../../main/common/rx/depend/core/CallState';
...
function calcCheckResult(call) {
  ...
  if (state._this === ALWAYS_CHANGE_VALUE) {
    sum = ALWAYS_CHANGE_VALUE;
  } else {
    sum = sumArgs;
    ...
    for (let i = 0, len = dependencies.length; i < len; i++) {
      ...
      if (typeof dependencyState.value !== 'undefined' && dependencyState.value !== ALWAYS_CHANGE_VALUE) {
...
function checkCallResult(call, result, isLazy) {
  if (typeof result === 'undefined') {
    ...
    assert.ok(result === ALWAYS_CHANGE_VALUE || Number.isFinite(result));
  }
...
function _stressTest({
  ...
  function createDependFunc() {
    ...
    const func = function () {
      ...
      checkDependenciesIsEmpty(state);
      const sumArgs = calcSumArgs.apply(_this, arguments); // for (let i = 0, len = arguments.length; i < len; i++) {
      // 	sumArgs += arguments[i]
      // }
  ...
  function _createCall(level) {
    ...
    const _this = rnd.nextBoolean(0.05) ? ALWAYS_CHANGE_VALUE : rnd.nextInt(1, 4);
