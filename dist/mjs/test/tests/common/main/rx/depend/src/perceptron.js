...
export function createPerceptron(layerSize, layersCount, check = true) {
  const countFuncs = layersCount * layerSize + 2; // region randomValues

  const randomValues = [0, {
    x: 1
  }, '123', null, () => ({}), 1, false, () => 1, {}, '', void 0, 2, {
    y: 1,
    x: 2
  }, true];
  const randomValuesLength = randomValues.length; // endregion
  ...
  for (let i = 0; i < layersCount - 1; i++) {
    ...
    for (let j = 0; j < layerSize; j++) {
      const prevLayer = layer; // const r1 = randomValues[(i * layerSize * 3 + j) % randomValuesLength]
  ...
  const getStates = () => {
    if (!_states) {
      _states = layers.flatMap(o => o).map(o => getOrCreateCallState(o)());
    }
