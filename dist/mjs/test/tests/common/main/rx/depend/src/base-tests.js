...
import { isAsync, isThenable } from '../../../../../../../main/common/async/async';
import { resolveAsync, ThenableSync } from '../../../../../../../main/common/async/ThenableSync';
import { deleteCallState, getOrCreateCallState, statusToString } from '../../../../../../../main/common/rx/depend/core/CallState';
...
if (typeof global !== 'undefined') {
  // for debug only
  global.statusToString = statusToString;
} // region baseTest
...
function getCallId(funcId, _this, ...rest) {
  ...
  callId += '(' + (Array.isArray(_this) ? _this.filter(o => !o.hasLoop).map(o => o.id).join(',') : _this || 0) + ')';
  return callId;
}

let resultsAsError = [];

function setResultsAsError(...calls) {
  resultsAsError = calls.map(o => o.id);
}

let alwaysChange = [];

function setAlwaysChange(...calls) {
  alwaysChange = calls.map(o => o.id);
}

let lazyCalls = [];

function setLazyCalls(...calls) {
...
function funcSync(id, deferred) {
  const result = depend(function () {
    ...
    assert.strictEqual(currentState.id, callId);
...
function funcSyncIterator(id, deferred) {
  const nested = function* (dependencies, currentState, isLazy) {
    assert.strictEqual(getCurrentState(), currentState);
    yield 1;
    assert.strictEqual(getCurrentState(), currentState);
  ...
  const run = function* (callId, dependencies, currentState) {
    assert.strictEqual(getCurrentState(), currentState);
    yield 1;
    assert.strictEqual(getCurrentState(), currentState);
  ...
  const result = depend(function () {
    ...
    const currentState = getCurrentState();
    assert.strictEqual(currentState.id, callId);
    const res = run(callId, this, currentState);
    assert.strictEqual(getCurrentState(), currentState);
    return res;
  }, !deferred ? null : getDeferredOptions(false));
  result.id = id;
...
function funcAsync(id, deferred) {
  ...
  const nestedAsync = function* (dependencies, currentState, isLazy) {
    assert.strictEqual(getCurrentState(), currentState);
    yield 1;
    assert.strictEqual(getCurrentState(), currentState);
...
function _funcCall(func, callId, _this, ...rest) {
  const result = () => {
    const currentState = getCurrentState();
...
function clearState(call) {
  ...
  const callId = oldState.id;
  let oldArgs;
  oldState.callWithArgs(null, function () {
    oldArgs = Array.from(arguments);
  });
  deleteCallState(oldState);
  const newState = oldState.callWithArgs(oldState._this, getOrCreateCallState(oldState.func));
  assert.ok(newState);
...
function checkDependenciesDuplicates(...funcCalls) {
  const set = new Set();
  ...
  for (let i = 0, len = funcCalls.length; i < len; i++) {
    ...
    if (_unsubscribers != null) {
      for (let j = 0, len = _unsubscribersLength; j < len; j++) {
        const id = _unsubscribers[j].state.id;
        assert.ok(id, state.id);
        assert.notOk(set.has(id), `Duplicate ${id} in ${state.id}`);
        set.add(id);
      }
...
function checkFuncSync(resultType, funcCall, ...callHistory) {
  checkCallHistory();
...
function checkFuncAsync(resultType, funcCall, ...callHistory) {
  checkCallHistory();
  checkDependenciesDuplicates(funcCall);
  assert.strictEqual(getCurrentState(), null);
  let thenable;
  ...
  try {
    thenable = checkAsync(funcCall()).then(o => {
      assert.strictEqual(getCurrentState(), null);
      return o;
    }, o => {
      assert.strictEqual(getCurrentState(), null);
  ...
  return (async () => {
    ...
    if (resultType === ResultType.Error || resultType === ResultType.Any && error) {
      assert.ok(error); // if (resultsAsError.indexOf(error + '') < 0) {
      // 	assert.fail(`funcCall.id = ${funcCall.id}, error = ${error}`)
      // }
    } else if (resultType === ResultType.Value || resultType === ResultType.Any && !error) {
      assert.notOk(funcCall.state.error);
...
function checkStatuses(...funcCalls) {
  return function (...statusesShort) {
    assert.deepStrictEqual(funcCalls.map(o => statusToShortString(o.state.status)), statusesShort);
  };
}

function _invalidate(...funcCalls) {
  checkCallHistory();
...
function checkSubscribers(funcCall, ...subscribersFuncCalls) {
  const ids = getSubscribers(funcCall.state).map(o => o.id).sort();
  const checkIds = subscribersFuncCalls.map(o => o.id).sort();
  assert.deepStrictEqual(ids, checkIds, funcCall.id);
}

function checkUnsubscribers(funcCall, ...unsubscribersFuncCalls) {
  const ids = funcCall.state._unsubscribers === null || funcCall.state._unsubscribersLength === 0 ? [] : funcCall.state._unsubscribers.filter(o => o).map(o => o.state.id).sort();
  const checkIds = unsubscribersFuncCalls.map(o => o.id).sort();
  assert.deepStrictEqual(ids, checkIds, funcCall.id);
}

function checkCurrentStateAsync(state) {
  assert.strictEqual(getCurrentState(), state);
  return resolveAsync(resolveAsync(delay(0), () => {
    assert.strictEqual(getCurrentState(), state);
    throw 1;
  }, () => {
    assert.fail();
...
export async function baseTest(deferred) {
  if (deferred == null) {
  ...
  assert.strictEqual(AL5.id, 'AL55()'); // endregion
  // region sync
  ...
  }, InternalError, /\bsync loop\b/i); // checkCallHistory(IL4, IL3, IL5)

  assert.throws(() => {
    checkFuncSync(ResultType.Value, IL5, IL5);
  }, InternalError, /\bsync loop\b/i); // checkCallHistory(IL5, IL4, IL3)
  // endregion
  // region async
...
export async function lazyTest(deferred) {
  if (deferred == null) {
  ...
  const _checkStatuses = checkStatuses(...allFuncs); // function _checkSubscribersAll() {
  // 	checkSubscribers(S0, S1)
  // 	checkSubscribers(I0, S1, I1)
  // 	checkSubscribers(A0, I1)
  // 	checkSubscribers(S1, S2, I2)
  ...
  assert.strictEqual(AL2.id, 'AL2(A(_))'); // endregion
  // region base tests
  // SL
  ...
  await promise2;
  await A0();
  await delay(100); // TODO: test result is different sometimes
  ...
  await promise1; // _checkStatuses('CV',  'IrV', 'IrV',   'CV', 'IrV', 'ca', 'IrV')

  await promise2; // _checkStatuses('CV',  'IrV', 'IrV',   'CV', 'IrV', 'CV', 'IrV')
