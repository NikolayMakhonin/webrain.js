...
describe('common > main > rx > properties > CalcObjectBuilder', function () {
  ...
  new CalcObjectBuilder(ValueObject.prototype).writable('value');
  ...
  class ClassSync extends ObservableClass {
    constructor({
      ...
      super();
      this.value = 'Value';
      this.source1 = new ValueObject(100);
      this.source2 = new ValueObject(0);
  ...
  let calcCount = 0;
  new CalcObjectBuilder(ClassSync.prototype).writable('source1').writable('source2').writable('source1_').writable('source2_').calc('calc1', connectorFactory({
    buildRule: c => c.connect('connectValue1', b => b.v('lastOrWait').p('source1', 'source1_').v('wait')).connect('addCalcCount', b => b.p('addCalcCount'))
  ...
  })).calc('calc2', connectorFactory({
    buildRule: c => c.connect('connectValue1', b => b.path(o => o['@lastOrWait']['source2|source2_']['@wait'])).connect('addCalcCount', b => b.p('addCalcCount'))
  }), calcPropertyFactory({
    dependencies: d => d.invalidateOn(b => b.p('connectValue1').p('value')),
  ...
  })).calc('calcWithAnyRule', connectorFactory({
    buildRule: c => c
  }), calcPropertyFactory({
    dependencies: d => d.invalidateOn(b => b.any(b2 => b2.p('prop1'), b2 => b2.p('prop2'))),
  ...
  new CalcObjectBuilder(ClassAsync.prototype).calc('calc1', connectorFactory({
    buildRule: c => c.connect('connectValue1', b => b.path(o => o['@lastOrWait']['source1|source1_']['@wait'])).connect('addCalcCount', b => b.p('addCalcCount'))
  }), calcPropertyFactory({
    dependencies: d => d.invalidateOn(b => b.p('connectValue1').p('value')),
  ...
  it('calc async', async function () {
    ...
    assert.deepStrictEqual((await object.wait), new Date(100));
    assert.deepStrictEqual(object.last, new Date(100));
    object = new ClassAsync().calc1;
    assert.deepStrictEqual((await object.lastOrWait), new Date(100));
    assert.deepStrictEqual(object.last, new Date(100));
  });
  ...
  it('deepSubscribe calcWithAnyRule sync', function () {
    ...
    new TestDeepSubscribe({
      ...
      immediate: true,
      doNotSubscribeNonObjectValues: true
    }, b => b.p('calcWithAnyRule')).subscribe([new Date(1)]).unsubscribe([new Date(1)]).subscribe([new Date(1)]).unsubscribe([new Date(1)]);
  });
  it('deepSubscribe calc sync', function () {
    for (let subscribed = 1; subscribed >= 0; subscribed--) {
      ...
      new TestDeepSubscribe({
        ...
        immediate: true,
        doNotSubscribeNonObjectValues: true
      }, b => b.p('calc1')).subscribe([new Date(101)]).unsubscribe([new Date(101)]).subscribe([new Date(101)]).unsubscribe([new Date(101)]).change(o => calcCount -= subscribed ? 2 : 3, [], []).change(o => o.source1 = new ValueObject(100), [], []).subscribe([new Date(101)]).unsubscribe([new Date(101)]) // .change(o => calcCount -= subscribed, [], [])
      .change(o => {
        o.source1_ = o.source1;
        o.source1 = void 0;
      }, [], []).subscribe([new Date(101)]).change(o => o.source1_.value = 200, [new Date(101)], [new Date(202)]).unsubscribe([new Date(202)]);
      new TestDeepSubscribe({
        object: new ClassSync({
          subscribed: !!subscribed,
          addCalcCount: true
        ...
        immediate: true,
        doNotSubscribeNonObjectValues: true
      }, b => b.p('calc1').p('getTime')).subscribe([Date.prototype.getTime]).change(o => o.source1.value = 200, [Date.prototype.getTime], [Date.prototype.getTime]).unsubscribe([Date.prototype.getTime]);
    }
  });
  xit('deepSubscribe calc async', async function () {
    ...
    for (let subscribed = 1; subscribed >= 0; subscribed--) {
      ...
      await tester.subscribeAsync([new Date(101)]);
      await tester.unsubscribeAsync([new Date(101)]); // await tester.changeAsync(o => calcCount -= subscribed, [], [])

      await tester.changeAsync(o => {
        o.source1_ = o.source1;
        o.source1 = void 0;
      }, [], []);
      await tester.subscribeAsync([new Date(101)]);
      ...
      await tester.changeAsync(o => o.source1.value = 200, [Date.prototype.getTime], [Date.prototype.getTime]);
  ...
  it('deepSubscribe calc circular sync', function () {
    ...
    for (let subscribed = 1; subscribed >= 0; subscribed--) {
      ...
      b => b.p('calc2').p('value').p('calc1')) // .subscribe([new Date(100)])
      // .unsubscribe([new Date(100)])
      .subscribe([new Date(101)]).unsubscribe([new Date(101)]).subscribe([new Date(101)]).unsubscribe([new Date(101)]).change(o => calcCount -= subscribed ? 2 : 3, [], []).change(o => o.source1 = new ValueObject(100), [], []).subscribe([new Date(101)]).unsubscribe([new Date(101)]) // .change(o => calcCount -= subscribed, [], [])
      .change(o => {
        o.source1_ = o.source1;
        o.source1 = void 0;
      }, [], []).subscribe([new Date(101)]).change(o => o.source1_.value = 200, [new Date(101)], [new Date(202)]).change(o => o.source2.value += 10, [new Date(202)], [new Date(202)]).change(o => o.source1_.value = 300, [new Date(202)], [new Date(303)]).unsubscribe([new Date(303)]);
    }
  });
