...
export class TestList extends TestVariants {
  constructor() {
    super();
  ...
  testVariant(options) {
    ...
    for (let debugIteration = 0; debugIteration < 3; debugIteration++) {
      ...
      try {
        ...
        const arrayReplicate = options.autoSort ? array.slice(0, list.size).sort(compare || compareFast) : array.slice(0, list.size); // assert.strictEqual(
        // 	list.countSorted,
  ...
  static test(testCases) {
    ...
    if (!testCases.countSorted && maxArrayLength <= 1 && (!testCases.compare || testCases.compare.length <= 0)) {
      const compare = testCases.compare && testCases.compare.length && testCases.compare[0] || compareFast;
      let minCountSorted;
