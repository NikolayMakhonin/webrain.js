...
xdescribe('time-limits', function () {
  ...
  function addTimeLimits(result, prefix, timeLimit) {
    ...
    add(prefix + 'new TimeLimits(...<complex>...)', key => new TimeLimits(new TimeLimits(new TimeLimits(timeLimit(key))), new TimeLimits(new TimeLimits(timeLimit(key), timeLimit(key))), new TimeLimits(new TimeLimits(timeLimit(key), timeLimit(key), new TimeLimits(new TimeLimits(timeLimit(key)), new TimeLimits(timeLimit(key), timeLimit(key)))))));
  }
    ...
    await timeLimitsIterator(5, 100, async timeLimit => {
      ...
      for (let i = 2; i--;) {
        ...
        try {
          ...
          await test(null, 2);
          await test(null, 2);
          await test(null, 2); // console.log(JSON.stringify(JSON.stringify(timeLimit.debug), null, 4))

          assert.strictEqual(callCount, 5); // , `i = ${i}\r\n${JSON.stringify(timeLimit.debug)}`)

          callCount = 0;
          await Promise.all([test(100, 300)]);
          ...
          callCount = 0;
          await delay(300); // console.log(JSON.stringify(JSON.stringify(timeLimit.debug), null, 4))
