...
export class PairingHeap {
  ...
  add(item) {
    let node = this._objectPool != null ? this._objectPool.get() : null;
  ...
  deleteMin() {
    ...
    const result = _root == null ? void 0 : _root.item;
    this.delete(_root);
    return result;
  }
  /**
...
export function merge(a, b, lessThanFunc) {
  let parent;
  let child;
...
export function collapse(node, lessThanFunc) {
  let tail;
  let a;
  let b;
  let next;
