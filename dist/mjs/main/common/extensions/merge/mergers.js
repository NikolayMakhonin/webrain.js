  ...
  _createClass(ValueState, [{
    ...
    value: function canMerge(source, target) {
      ...
      if (canMerge) {
        ...
        if (typeof result !== 'boolean') {
          throw new Error("Unknown canMerge() result (".concat(result.constructor.name, ") for ").concat(this.type.name));
        }
    ...
    get: function get() {
      ...
      if (_cloneInstance == null) {
        ...
        if (!_cloneInstance) {
          throw new Error("Class (".concat(typeToDebugString(type), ") cannot be clone"));
        }

        if (_cloneInstance === target) {
          throw new Error("Clone result === Source for (".concat(typeToDebugString(type), ")"));
        }

        if (_cloneInstance.constructor !== type) {
          throw new Error("Clone type !== (".concat(typeToDebugString(type), ")"));
        }
...
export var TypeMetaMergerCollection =
/*#__PURE__*/
function (_TypeMetaCollection) {
  ...
  function TypeMetaMergerCollection(proto) {
    ...
    return _possibleConstructorReturn(this, _getPrototypeOf(TypeMetaMergerCollection).call(this, proto || TypeMetaMergerCollection.default));
  }
...
registerMerger(Object, {
  ...
  preferClone: function preferClone(o) {
    return Object.isFrozen(o) ? true : null;
  }
}); // endregion
// region Date
    ...
    merge: function merge(_merge5, base, older, newer, set, preferCloneOlder, preferCloneNewer, options) {
      return mergeMaps(function (target, source) {
        return createMergeSetWrapper(target, source, function (arrayOrIterable) {
          return fillSet(new Set(), arrayOrIterable);
        });
    ...
    merge: function merge(_merge6, base, older, newer, set, preferCloneOlder, preferCloneNewer, options) {
      return mergeMaps(function (target, source) {
        return createMergeMapWrapper(target, source, function (arrayOrIterable) {
          return fillMap(new Map(), arrayOrIterable);
        });
