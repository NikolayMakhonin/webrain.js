...
function calcSumArgs() {
  ...
  if (this !== _CallState.ALWAYS_CHANGE_VALUE) {
    sum += this;
  }
  ...
  for (var i = 0, len = arguments.length; i < len; i++) {
    ...
    if (arg !== _CallState.ALWAYS_CHANGE_VALUE) {
      sum += arg;
    }
  }
...
function calcCheckResult(call) {
  ...
  if (state._this === _CallState.ALWAYS_CHANGE_VALUE) {
    sum = _CallState.ALWAYS_CHANGE_VALUE;
  } else {
    sum = sumArgs;
    ...
    for (var i = 0, len = dependencies.length; i < len; i++) {
      ...
      if (typeof dependencyState.value !== 'undefined' && dependencyState.value !== _CallState.ALWAYS_CHANGE_VALUE) {
...
function checkCallResult(call, result, isLazy) {
  if (typeof result === 'undefined') {
    ...
    _Assert.assert.ok(result === _CallState.ALWAYS_CHANGE_VALUE || (0, _isFinite.default)(result));
  }
...
function runLazy(rnd, state, sumArgs, countDependencies, getNextCall) {
  ...
  for (var i = 0; i < countDependencies; i++) {
    ...
    if (typeof result !== 'undefined' && result !== _CallState.ALWAYS_CHANGE_VALUE) {
      sum += result;
    }
  }
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 22:
          ...
          return _context.abrupt("return", _CallState.ALWAYS_CHANGE_VALUE);
...
function _stressTest(_ref4) {
  ...
  function createDependFunc() {
    ...
    var func = function func() {
      ...
      checkDependenciesIsEmpty(state);
      var sumArgs = calcSumArgs.apply(_this, arguments); // for (let i = 0, len = arguments.length; i < len; i++) {
      // 	sumArgs += arguments[i]
      // }
  ...
  function _createCall(level) {
    ...
    var _this = rnd.nextBoolean(0.05) ? _CallState.ALWAYS_CHANGE_VALUE : rnd.nextInt(1, 4);
