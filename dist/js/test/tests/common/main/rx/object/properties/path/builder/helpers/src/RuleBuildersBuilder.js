...
// endregion
var RuleBuildersBuilder = /*#__PURE__*/function () {
    ...
    value: function _if() {
      ...
      this.ruleFactoriesTree.push(function If(index) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          index = indexNext;
          args.push((0, _isArray.default)(conditionRule) ? [conditionRule[0], ruleFactory] : ruleFactory);
        }
...
function ruleFactoriesVariants() {
  var _RuleBuildersBuilder;

  var ruleFactoriesTree = (_RuleBuildersBuilder = new RuleBuildersBuilder()).variants.apply(_RuleBuildersBuilder, arguments).ruleFactoriesTree;

  var factoriesVariants = (0, _Variants.treeToSequenceVariants)(ruleFactoriesTree);
  var factories = [];

  var _loop = function _loop() {
    var factoriesVariant = _step.value;
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(factoriesVariants), _step; !(_step = _iterator()).done;) {
    _loop();
  }
